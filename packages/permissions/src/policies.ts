import type { RiskLevel, SecurityEvaluation, ToolPolicy, ToolRequest } from '@wasp/shared-types';
import { RISK_ORDER } from '@wasp/shared-types';

/**
 * Tool policy registry. THIS IS DATA, NOT PROMPT.
 *
 * The model can ask for any tool name it wants; this table decides the risk.
 * Unknown tools fall back to DEFAULT_POLICY (HIGH, approval required) so the
 * system fails closed.
 *
 * Tool names follow the ORANGE proposal (INSTRUCCIONESFELIPE.md "TOOL REGISTRY").
 * Felipe: if you rename a tool, update it here or call registerPolicy().
 */
export const DEFAULT_TOOL_POLICIES: ToolPolicy[] = [
  ...
  { tool: 'docker_status', risk_level: 'LOW', requires_approval: false, description: 'Check whether Docker is available (no side effects)' },
  ...
  { tool: 'sandbox_http_local', risk_level: 'MEDIUM', requires_approval: true, description: 'HTTP request to a local test service from the sandbox', allowed_agents: ['operator'] },
  ...
  { tool: 'sandbox_external_network', risk_level: 'HIGH', requires_approval: true, description: 'Give the sandbox Internet access', allowed_agents: ['operator'] },
...
const PRIVATE_OR_LOCAL = /^(localhost|127\.\d+\.\d+\.\d+|::1|10\.\d+\.\d+\.\d+|192\.168\.\d+\.\d+|172\.(1[6-9]|2\d|3[01])\.\d+\.\d+|[a-z0-9-]+\.(local|lab|internal|test))$/i;

const ESCALATIONS: Escalation[] = [
  {
    name: 'external_network_flag',
    test: (r) => str(r.input['network_access']) === 'external' || r.input['external'] === true,
    min_risk: 'HIGH',
  ...
  },
  {
    ...
    test: (r) => {
      const target = str(r.input['target']) ?? str(r.input['host']) ?? str(r.input['url']);
...
export class ToolPolicyRegistry {
  ...
  evaluate(request: ToolRequest): SecurityEvaluation {
    const policy = this.policies.get(request.tool) ?? DEFAULT_POLICY;
    ...
    if (policy === DEFAULT_POLICY) {
      notes.push(`tool "${request.tool}" is not in the registry`);
    } else {
      notes.push(policy.description);
    }

    if (policy.allowed_agents && !policy.allowed_agents.includes(request.agent)) {
      blocked = true;
      risk = 'CRITICAL';
      notes.push(`agent "${request.agent}" is not allowed to request "${request.tool}"`);
    }
