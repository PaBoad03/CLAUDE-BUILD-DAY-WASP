import type { VoiceDecision } from '@wasp/shared-types';

/**
 * Interprets a human utterance (STT transcript or typed text) as an
 ...
 *
 * This is the ONLY place that turns speech into AUTHORIZED. The voice layer
 * (MAGENTA / Andrea) gives us text; we never listen to audio here.
 */
...
const YES_PHRASES = new Set([
  // es
  'si', 'si procede', 'si adelante', 'si autorizo', 'si autorizado', 'si confirmo', 'si continua', 'si hazlo', 'si puedes', 'si puedes proceder',
...
const NO_PHRASES = new Set([
  // es
  'no', 'no procedas', 'no lo hagas', 'no autorizo', 'no autorizado', 'denegado', 'deniego', 'lo deniego', 'rechazado', 'rechazo', 'no lo apruebo', 'no continues', 'negativo', 'no gracias', 'no por ahora', 'no ahora',
  // en
  'no', 'do not proceed', 'dont proceed', 'do not', 'dont', 'dont do it', 'do not do it', 'denied', 'deny', 'i deny', 'not authorized', 'not approved', 'rejected', 'reject', 'negative', 'no thanks', 'no thank you', 'not now', 'permission denied',
]);

/** Cancel / stop words: if ANY appears, the operation is CANCELLED. Safety first. */
...
export function interpretAuthorization(text: string): VoiceInterpretation {
  ...
  for (const tok of tokens) {
    if (CANCEL_WORDS.has(tok)) {
      return { decision: 'CANCELLED', normalized, reason: `cancel word "${tok}"` };
    }
  }
  for (const phrase of CANCEL_WORDS) {
    if (phrase.includes(' ') && normalized.includes(phrase)) {
      return { decision: 'CANCELLED', normalized, reason: `cancel phrase "${phrase}"` };
    }
  }

  // 2. Exact known phrases.
  if (NO_PHRASES.has(normalized)) {
    return { decision: 'DENIED', normalized, reason: 'exact deny phrase' };
  }
  if (YES_PHRASES.has(normalized)) {
    return { decision: 'AUTHORIZED', normalized, reason: 'exact authorize phrase' };
  }
  ...
  if (hasYes && hasNo) {
    return { decision: 'AMBIGUOUS', normalized, reason: 'contains both affirmative and negative words' };
  }
  ...
  //    We only accept when the utterance is short (<= 6 tokens) to avoid long rambling being read as consent.
  if (tokens.length <= 6) {
    const first = tokens[0]!;
    if (NO_TOKENS.has(first) && !hasYes) {
      return { decision: 'DENIED', normalized, reason: `short answer starting with "${first}"` };
    }
    if (YES_TOKENS.has(first) && !hasNo) {
      return { decision: 'AUTHORIZED', normalized, reason: `short answer starting with "${first}"` };
    }
  }
