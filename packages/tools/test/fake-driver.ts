import type { ContainerState, DockerAvailability, ExecOutcome, SandboxDriver } from '../src/docker.js';
...
export class FakeDriver implements SandboxDriver {
  ...
  async containerState(name: string) { return name === 'wasp-sandbox' ? this.sandbox : this.target; }
  async imageExists() { return true; }
  async buildSandboxImage() { return ok('built'); }
  async pullTargetImage() { return ok('pulled'); }
  async ensureLabNetwork() { /* noop */ }
  async ensureTarget() { this.target = 'running'; }
  async createSandbox() { this.sandbox = 'running'; this.networks = ['wasp-lab']; return ok('abc123'); }
  async destroySandbox() { this.sandbox = 'absent'; this.networks = []; return ok('wasp-sandbox'); }
  async connectExternal() { this.networks.push('bridge'); return ok(); }
  async sandboxNetworks() { return this.networks; }
