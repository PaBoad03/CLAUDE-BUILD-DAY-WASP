...
test('sandbox_create then ping parses packet loss and reports exact argv', async () => {
  const { driver, exec } = setup();
  const c = await exec.execute({ request_id: 'r3', tool: 'sandbox_create', args: {} });
...
test('100% packet loss is a failure, not a success', async () => {
  const { driver, exec } = setup();
  await exec.execute({ request_id: 'r5', tool: 'sandbox_create', args: {} });
...
test('rejected args never reach the driver', async () => {
  const { driver, exec } = setup();
  await exec.execute({ request_id: 'r7', tool: 'sandbox_create', args: {} });
...
test('dns, http, port, ifaces, routes all parse fake output', async () => {
  const { exec } = setup();
  await exec.execute({ request_id: 'c', tool: 'sandbox_create', args: {} });
