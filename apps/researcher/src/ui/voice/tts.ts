/**
 * Browser text-to-speech for the MAGENTA agent.
 *
 * Candidate for `packages/voice` (owner per CODEOWNERS: Andrea). Kept
 * dependency-free (Web Speech API) so every PC can run it without keys.
 * Browsers require a user gesture before audio; call `enable()` from a click.
 *
 * `onUtterance` lets the host report `tts_started` / `tts_finished` to the bus.
 */
